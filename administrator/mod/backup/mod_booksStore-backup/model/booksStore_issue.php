...
class model_booksStore_issue extends JDatabaseMySQL{  			
          var $qry = '';  
		  var $_result = '';             
		  var $searchCondition = ''; 
		   var $originalBanner = '';
		...
		function remove_product($pid){
			...
			for($i=0;$i<$max;$i++){
				if($pid==$_SESSION['issue_cart'][$i]['productid']){
					unset($_SESSION['issue_cart'][$i]);
					break;
				}
			}
			$_SESSION['issue_cart']=array_values($_SESSION['issue_cart']);
		}
		...
		for($i=0;$i<$max;$i++){
			...
			$price=model_booksStore_issue::get_price($pid);
			$sum+=$price*$q;
		}
		return $sum;
	}
