...
/*                 FOR PAGINATION INITIALIZATION THE ABOVE VARIABLES
				   1. Call $data = $this->pending_reportnavigation();
...
/*                 Load records from database 
				   $Data = $this->pending_reportnavigation();  
...
class model_reports_issuebook extends JDatabaseMySQL{  			
          var $qry = '';  
		  var $_result = '';             
		  var $searchCondition = ''; 
		   var $originalBanner = '';
		  ...
		  function get_all_businesscategories()
		{
		 ...
		 function get_all_empname($id)
		{
			$this->sql = "SELECT user_display_name FROM mgl_users where user_id='$id'";	
			$this->query();
			return $this->loadAssoc();
		...
		function get_all_bookswriters()
		{
			...
			function initialize(){
				...
				$this->tb_name='mgl_total_amount a,mgl_issue_book b,mgl_users as c';
				 $this->where = '  where b.issue_type=1 and a.amt_id=b.issue_foregin_id and c.user_id=a.emp_issue_id and '.$this->searchCondition.'  group by b.issue_foregin_id order by a.issue_user_id asc';
				...
				if(isset($_REQUEST['particularsearch']) && $_REQUEST['particularsearch']=='particularsearch'){
					...
					if(isset($_REQUEST['sdate']) && $_REQUEST['sdate']!=''){
						$this->page = 'index.php?mod=mod_reports&view=issuebook&m=6&filtertype='.$_REQUEST['filtertype'].'&report_datewise='.$_REQUEST['report_datewise'].'&sdate='.$_REQUEST['sdate'].'&edate='.$_REQUEST['edate']."&cmbStatus=".$_REQUEST['cmbStatus'];
					}
				}
				else if(isset($_REQUEST['report_yearwise']) && @$_REQUEST['report_yearwise']=='report_yearwise'){
				$year1=$_REQUEST['yearselection'];
				...
				$secondhalf_end=$year1."-12-31";
					if(isset($_REQUEST['typeselection']) && @$_REQUEST['typeselection']==3){
					$this->page = 'index.php?mod=mod_reports&view=issuebook&m=6&yearselection='.$_REQUEST['yearselection'].'&report_yearwise='.$_REQUEST['report_yearwise'].'&typeselection='.$_REQUEST['typeselection'];
					}
					else if(isset($_REQUEST['typeselection']) && @$_REQUEST['typeselection']==1){
								if(isset($_REQUEST['quarter']) && @$_REQUEST['quarter']==1){
								$this->page = 'index.php?mod=mod_reports&view=issuebook&m=6&yearselection='.$_REQUEST['yearselection'].'&report_yearwise='.$_REQUEST['report_yearwise'].'&quarter='.$_REQUEST['quarter'].'&typeselection='.$_REQUEST['typeselection'];
						 }
						 else if(isset($_REQUEST['quarter']) && @$_REQUEST['quarter']==2){
								$this->page = 'index.php?mod=mod_reports&view=issuebook&m=6&yearselection='.$_REQUEST['yearselection'].'&report_yearwise='.$_REQUEST['report_yearwise'].'&quarter='.$_REQUEST['quarter'].'&typeselection='.$_REQUEST['typeselection'];
						 }
						...
						if(isset($_REQUEST['selecthalf']) && @$_REQUEST['selecthalf']==1){
								$this->page = 'index.php?mod=mod_reports&view=issuebook&m=6&yearselection='.$_REQUEST['yearselection'].'&report_yearwise='.$_REQUEST['report_yearwise'].'&selecthalf='.$_REQUEST['selecthalf'].'&typeselection='.$_REQUEST['typeselection'];
						 }
						 else if(isset($_REQUEST['selecthalf']) && @$_REQUEST['selecthalf']==2){
								$this->page = 'index.php?mod=mod_reports&view=issuebook&m=6&yearselection='.$_REQUEST['yearselection'].'&report_yearwise='.$_REQUEST['report_yearwise'].'&selecthalf='.$_REQUEST['selecthalf'].'&typeselection='.$_REQUEST['typeselection'];
						 }
				...
				$this->page = 'index.php?mod=mod_reports&view=issuebook&m=6';
				}
				$this->pageLimit = '10';
				// $this->_result = mysql_query($this->qry);
                     return $this->pending_reportnavigation();
				...
				function getvarp(){
				 //$this->_result = mysql_query($this->qry);
						$_data = $this->pending_reportnavigation();  
		...
		{
			$this->sql = "SELECT sum(book_qty) as totalbooks FROM mgl_issue_book where issue_type=1 and issue_foregin_id='$id'";	
			$this->query();
			return $this->loadAssoc();
		}
