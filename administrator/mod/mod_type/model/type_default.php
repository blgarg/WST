...
class model_type_default extends JDatabaseMySQL{  			
          var $qry = '';  
		  var $_result = '';             
		  var $searchCondition = ''; 
		   var $originalBanner = '';
	...
	//echo "<pre>zsdgasd"; print_r($_REQUEST); die;
			...
			$_REQUEST['creteria'] = substr($_REQUEST['creteria'],0,60);
					...
					$this->searchCondition .= ' and (type_name like "%'.trim($_REQUEST['creteria']).'%"  )'; 
					}
				...
				function deleteData($ids){
					...
					$this->sql = "delete FROM type WHERE id  in(".$ids.") ";
				...
				//echo $type; die;
					...
					$this->qry = "update type set status='".$typeval."' WHERE id in(".$modelIds.") ";
					$this->sql = $this->qry;
