...
function checkAddPage()
{
	with (window.document.adddPageFrm) { 
	    if (isEmpty(page_title, 'Please enter page title')) {
			return false;
		}else if (isEmpty(page_meta_tags, 'Please enter meta title')) {
			return false;
		}else if (validmeta(page_meta_tags,'Please enter valid meta title')) {
			return false;
		}else if (isEmpty(meta_desc, 'Please enter meta description')) {
			return false;
		}else if (validmetades(meta_desc,'Please enter valid meta description')) {
			return false;
		}
		else if(isLen(page_title,'151','More than 150 characters are not allowed.')){
			return false;
		}else {
			submit();
		}
