...
class model_travel_default extends JDatabaseMySQL{  			
	...
	//echo "<pre>zsdgasd"; print_r($_REQUEST); die;
			...
			$_REQUEST['creteria'] = strip_tags($_REQUEST['creteria']);
					...
					$this->searchCondition .= ' and (place_name like "%'.trim($_REQUEST['creteria']).'%"  )'; 
					}
				...
				$this->where .=' order by created_on desc';
			   	$this->adjacents = '1';
				$this->page = 'index.php?mod=mod_travel&view=default&m=4';
				$this->pageLimit = '10';
				...
				function deleteData($ids){
					...
					$this->qry = "delete FROM travel_guide WHERE id  in(".$ids.") ";
					$this->sql = $this->qry;
				...
				foreach ($ids as $id){
					...
					$this->query();
					$data = $this->loadAssoc();
					echo $image_path = $config_var->UPLOAD_ROOT."travel/".$data[0]['thumb_image']; ;
					echo $thumbnail_path = $config_var->UPLOAD_ROOT."travel/thumbs/".$data[0]['thumb_image']; ;
					@unlink($image_path);
				...
				//echo $type; die;
					...
					$this->qry = "update travel_guide set status='".$typeval."' WHERE id in(".$modelIds.") ";
					$this->sql = $this->qry;
