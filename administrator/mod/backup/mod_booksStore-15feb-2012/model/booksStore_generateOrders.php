...
class model_booksStore_generateOrders extends JDatabaseMySQL{  			
          var $categoryTitle;
		  var $categoryText;
		...
		$max=count($_SESSION['order']);
			  ...
			  $Books=model_booksStore_generateOrders::get_product_name($pid);
		...
		function issue_generate_order($data)
		{
	          $HelperOb = new Helper();
			  $config_var = new config();
			  $totalamount=@mysql_real_escape_string(stripslashes(number_format(model_booksStore_generateOrders::get_order_total(),2,".",",")));
			  $totalamount=str_replace(",", "", $totalamount) ;
			  $this->current_cart_price=@mysql_real_escape_string(stripslashes($data->current_cart_price));
			  $this->academy=@mysql_real_escape_string(stripslashes($data->academy));
			  $this->createdDate = date('Y-m-d g:i:s');
			  $this->newid='0';
			  $this->qry = "INSERT INTO tb_order_detail(
			          person_emp_id,
			  		  academy_id,	
					  order_date,
					  total_amt
			  ...
			  for($i=0;$i<$max;$i++){
					...
					//extract($row);
					if($q==0) continue;
					 $this->qry="INSERT INTO tb_order_send(
					  order_detail_id,
					  book_id,
					  book_qty,
					  book_name,
		...
		{
		$values=model_booksStore_generateOrders::check_is_quantity_available();
		if($values==1)
		{
			  $HelperOb = new Helper();
			  $config_var = new config();
		...
		for($i=0;$i<$max;$i++){
			$pid=$_SESSION['order'][$i]['productid'];
			$q=$_SESSION['order'][$i]['qty'];
			$price=model_booksStore_generateOrders::get_price($pid);
			$sum+=$price*$q;
		}
		return $sum;
	}
		function get_price($pid){
			...
			{
				$this->sql = "SELECT * FROM mgl_employee_users";
				$this->query();
				return $this->loadAssoc();
			}
			function get_all_business_type()
			{
				$this->sql = "SELECT * FROM mgl_business_parties where Actions=1";
				$this->query();
				return $this->loadAssoc();
			}
