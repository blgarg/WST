...
class model_booksStore_sell_books extends JDatabaseMySQL{  			
          var $categoryTitle;
		  var $categoryText;
		...
		$max=count($_SESSION['cart']);
			  ...
			  $Books=model_booksStore_sell_books::get_product_name($pid);
		...
		function issue_sell_books($data)
		{
		$values=model_booksStore_sell_books::check_is_quantity_available();
		if($values==1)
		{
		 $HelperOb = new Helper();
			  $config_var = new config();
			  $totalamount=number_format(model_booksStore_sell_books::get_order_total(),2,".",",");
			  $this->business_type=@mysql_real_escape_string(stripslashes($data->business_type));
			  $this->party_id=@mysql_real_escape_string(stripslashes($data->party_id));
			  $this->createdDate = date('Y-m-d g:i:s');
			  $this->newid='0';
			  $this->qry = "INSERT INTO mgl_amt_sell_books(
					  total_amt,
					  amt_date
					  ) VALUES
									('".$totalamount."',
			  ...
			  for($i=0;$i<$max;$i++){
					...
					//extract($row);
					if($q==0) continue;
					 $this->qry="INSERT INTO mgl_sell_books(
					  sell_amt_id,
					  person_id,
					  business_id_type,
					  party_id,
			...
			{
			@header('Location:index.php?mod=mod_booksStore&view=sell_books&m=2&r=5');
			}	
		...
		for($i=0;$i<$max;$i++){
			$pid=$_SESSION['cart'][$i]['productid'];
			$q=$_SESSION['cart'][$i]['qty'];
			$price=model_booksStore_sell_books::get_price($pid);
			$sum+=$price*$q;
		}
		return $sum;
	}
		function get_price($pid){
			...
			{
				$this->sql = "SELECT * FROM mgl_books_categories where Actions=1";
				$this->query();
				return $this->loadAssoc();
			}
