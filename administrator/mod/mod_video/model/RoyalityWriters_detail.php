...
class model_RoyalityWriters_detail extends JDatabaseMySQL{  			
          var $qry = '';  
		  var $_result = '';             
		  var $searchCondition = ''; 
		   var $originalBanner = '';
          ...
          function __construct(  ){ 
			...
			function initialize(){
				...
				if(isset($_REQUEST['hdnSearch']) && $_REQUEST['hdnSearch']=='hdnsearch'){
				   ...
				   $this->where = ' mgl_sell_books a,mgl_amt_sell_books b WHERE a.sell_amt_id=b.amt_book_id and a.writer_id='.base64_decode($_REQUEST['writer_id']).' group by a.sell_amt_id '.$this->searchCondition.'  order by a.sell_id desc';
				...
				else if(isset($_REQUEST['writer_id']))
				{
				$this->page = 'index.php?mod=mod_writers&view=detail&m=3&writer_id='.base64_decode($_REQUEST['writer_id']);
				}
				else{
				$this->page = 'index.php?mod=mod_writers&view=detail&m=3&writer_id='.base64_decode($_REQUEST['writer_id']);
				}
				...
				{
				$this->sql = "SELECT SUM(book_qty) as booksum FROM mgl_sell_books where sell_amt_id=$id and writer_id=".base64_decode($_REQUEST['writer_id']);	
				$this->query();
				return $this->loadAssoc();
				}
				function get_price_books($id)
				{
				$this->sql = "SELECT SUM(qty_price) as totalsum FROM mgl_sell_books where sell_amt_id=$id and writer_id=".base64_decode($_REQUEST['writer_id']);	
				$this->query();
				return $this->loadAssoc();
				}
				function get_price_balance($id)
				{
				$this->sql = "SELECT SUM(writer_amt) as balance FROM mgl_sell_books where sell_amt_id=$id and writer_id=".base64_decode($_REQUEST['writer_id']);	
				$this->query();
				return $this->loadAssoc();
				}
				function getvarp(){
