...
require('../../../../settings.php');
	...
	$str='SELECT * FROM mgl_business_parties WHERE parties_id="'.$partyid_info.'"';
	$rs=mysql_query($str);
	$row=mysql_fetch_array($rs);
...
require('../../../../settings.php');
	...
	$str='SELECT * FROM mgl_business_parties WHERE business_typeid="'.$_REQUEST['bill_partyid'].'"';
	$rs=mysql_query($str);
...
{
require('../../../../settings.php');
	$str='SELECT * FROM mgl_business_parties WHERE business_typeid="'.$_REQUEST['party'].'"';
	$rs=mysql_query($str);
	?>
    <option value="<?php base64_encode(0);?>">Select Party</option>
	 <?php while($result_value=mysql_fetch_array($rs))
	{?>
...
{
require('../../../../settings.php');
	$str='SELECT * FROM mgl_business_parties WHERE business_typeid="'.$_REQUEST['businesstypeid'].'"';
	$rs=mysql_query($str);
	?>
    <option value="<?php base64_encode(0);?>">Select Party</option>
	 <?php while($result_value=mysql_fetch_array($rs))
