...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        // Title
		$title=HARYANAGRANTHACADEMY."(".WRITERSREPORT.")";
...
$string='<table border="1" cellpadding="2" cellspacing="2" >
 <tr nobr="true" style="background-color:#E8E8E4;" align="center">
  <th width="120px;">'.WRITERNAME.'</th>
  <th width="90px;">'.EMPNAME.'</th>
  <th width="80px;">'.NUMBEROFBOOKSSOLD.'</th>
  <th width="80px;">'.ROYALITY.'</th>
  <th width="80px;">'.COUNTRY.'</th>
  <th  width="80px;">'.BIRTHDATE.'</th>
...
if(isset($_REQUEST['wid']) && $_REQUEST['wid']!='')
 ...
 $st="select * from mgl_writers where writer_id=".$wid." ORDER BY writer_id DESC";			 
$a=1;
...
//echo $fullname ;
 ...
 $total_books1=get_royality_prices($row['writer_id']);
 $total_books=explode(",",$total_books1);
			  if($total_books[1]!=''){$totals=RS."&nbsp;".number_format($total_books[1],2,".",","); } else{$totals=RS."&nbsp;0.00";}
if($total_books[0]==''){$qty=0;}else {$qty=$total_books[0];}
...
//$partytitle=get_aell_party($row['party_id']);
				$str[$a-1]='<tr nobr="true">
                    ...
                    <td align="center">'.$row['writer_country'].'</td>
                    <td align="center">'.$row['writer_dob'].'</td>
					<td align="center">'.$datewise[0].'</td>
                 </tr>';
$a++;
}
...
$pdf->Output('writersreport.pdf', 'I');
