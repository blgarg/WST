			...
			function initialize(){
				...
				$_REQUEST['creteria'] = substr($_REQUEST['creteria'],0,60); 
					...
					if(isset($_REQUEST['creteria']) && $_REQUEST['creteria']!='' && strtolower($_REQUEST['creteria'])!='search' ){
						$this->searchCondition .= ' and (video_title like "%'.trim($_REQUEST['creteria']).'%"  )';
					}
				...
				$this->adjacents = '1';
				if(isset($_REQUEST['hdnSearch']) && $_REQUEST['hdnSearch']=='hdnsearch'){
				$this->page = 'index.php?mod=mod_video&view=default&m=3&d=5&creteria='.trim($_REQUEST['creteria']).'&hdnSearch='.$_REQUEST['hdnSearch']."&cmbStatus=".$_REQUEST['cmbStatus'];
				}else{
				$this->page = 'index.php?mod=mod_video&m=3&view=default&d=5';
				}
				$this->pageLimit = '10';
				...
				$this->qry = "delete FROM videos WHERE id=".$id;
					$this->sql = $this->qry;
				...
				//echo $type; die;
					...
					$this->qry = "update videos set status='".$typeval."' WHERE id in(".$modelIds.") ";
					$this->sql = $this->qry;
