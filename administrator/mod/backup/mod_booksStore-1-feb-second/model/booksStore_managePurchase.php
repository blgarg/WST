...
class model_booksStore_managePurchase extends JDatabaseMySQL{  			
          var $qry = '';  
		  var $_result = '';             
		  var $searchCondition = ''; 
		   var $originalBanner = '';
          ...
          function __construct(  ){ 
			...
			function initialize(){
				...
				$this->where = ' where 1 '.$this->searchCondition.' and purchase_type=1 ORDER BY book_id DESC';
			    //$this->where = 'ORDER BY bannerId DESC';
				$this->adjacents = '1';
				if(isset($_REQUEST['hdnSearch']) && $_REQUEST['hdnSearch']=='hdnsearch'){
				$this->page = 'index.php?mod=mod_booksStore&view=managePurchase&m=1&d=5&creteria='.trim($_REQUEST['creteria']).'&hdnSearch='.$_REQUEST['hdnSearch']."&cmbStatus=".$_REQUEST['cmbStatus'];
				}else{
				$this->page = 'index.php?mod=mod_booksStore&view=managePurchase&m=1&d=5';
				}
				$this->pageLimit = '2';
