<?php
class Category_controller extends JDatabaseMySQL{	
	  var $modName  = '';
	    ...
	    switch(@$_REQUEST['controller'])
		  {
				...
				print_r($_REQUEST);
				die;*/
				$model_Category_addNewCategory = new model_Category_addNewCategory();
				global $config_var;
				...
				if(count($result)>0){
					...
					@header('Location:index.php?mod=mod_Category&view=addNewCategory&m=4');
						
				}else{
				if($model_Category_addNewCategory->inserData($objects)){
					$_SESSION['succ'] = "New category has been added successfully.";
					@header('Location:index.php?mod=mod_Category&view=default&m=4');
				}else{
					$_SESSION['error'] = "Error adding new category.";
					@header('Location:index.php?mod=mod_Category&view=default&m=4&r=1');
				 }
				 }
				break;
				case 'edit_category':
				...
				if(count($result)>0){
				$_SESSION['error']="Category title already exists";
				@header('Location:index.php?mod=mod_Category&view=editCategory&m=4&cat_id='.$objects->cat_id);
				...
				case 'delete':
				$model_booksCategory_delete = new model_Category_default();
				$objects = new stdClass();
				$objects->cat_id = @$_REQUEST['cat_id'];
				$modelIds = '0';
				...
				case 'activate':
				$model_booksCategory_active = new model_Category_default();
				$objects = new stdClass();
				...
				case 'deactivate':
				$model_booksCategory_deactive = new model_Category_default();
				$objects = new stdClass();
				$objects->cat_ids = @$_REQUEST['cat_id'];
				$modelIds = '0';
				if(count($objects->cat_ids)>0){
