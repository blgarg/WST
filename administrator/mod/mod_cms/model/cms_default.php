...
class model_cms_default extends JDatabaseMySQL{  			
          ...
          function __construct(  ){ 
			...
			function initialize(){
				...
				$this->tb_name = 'cms_pages';
				$this->where = ' where 1 '.$this->searchCondition.' ORDER BY page_id DESC';
			    //$this->where = 'ORDER BY bannerId DESC';
				$this->adjacents = '1';
				if(isset($_REQUEST['hdnSearch']) && $_REQUEST['hdnSearch']=='hdnsearch'){
				$this->page = 'index.php?mod=mod_cms&view=default&d=3&creteria='.trim($_REQUEST['creteria']).'&hdnSearch='.$_REQUEST['hdnSearch']."&cmbStatus=".$_REQUEST['cmbStatus'];
				}else{
				$this->page = 'index.php?mod=mod_cms&view=default&d=3';
				}
				$this->pageLimit = '10';
				...
				function deleteData($ids){
					$this->qry = "delete FROM mgl_pages WHERE page_id  in(".$ids.") ";
					$this->sql = $this->qry;
				...
				//echo $type; die;
					...
					$this->qry = "update mgl_pages set Actions='".$typeval."' WHERE page_id in(".$modelIds.") ";
					$this->sql = $this->qry;
