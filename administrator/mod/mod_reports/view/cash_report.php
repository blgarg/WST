...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        // Title
		$title=HARYANAGRANTHACADEMY."(".CASHREPORT.")";
...
include $config_var->WEB_ROOT.'administrator/inc/load.php';
			...
			if(@$_REQUEST['acapar'] ==1){
					$sql = "SELECT * FROM recive_payments where acadamy_id=$id order by id DESC";
			}else if(@$_REQUEST['acapar'] ==2){
					$sql = "SELECT * FROM recive_payments where party_id=$id order by id DESC";
			}
	$rst=mysql_query($sql);
	$resultarr = array();
	while($resultdata=mysql_fetch_array($rst))
	{		
...
$string='<table border="1" cellpadding="2" cellspacing="2" >
 <tr nobr="true" style="background-color:#E8E8E4;" align="center">
  <th width="90px;">'.BOOKORDERNUMBER.'</th>
  <th width="130px;">'.EMPNAME.'</th>
  <th width="70px;">'.PAIDAMOUNT.'</th>
	<th width="70px;">'.PENDINGAMOUNT.'</th>
	<th width="70px;">'.DISCOUNTAMOUNT.'</th>
  <th width="70px;">'.DATE.'</th>
  <th width="80px;">'.Quantity.'</th>
  <th width="70px;">'.TOTALAMOUNT.'</th>
...
for($p=0;$p<count($resultarr);$p++){
	
	$amtbookid = $resultarr[$p]['amt_book_id'];
	$paidamount = $resultarr[$p]['enter_payment'];
	$pendingpayment = $resultarr[$p]['pending_payment'];
	$paid_pending = $paidamount + $pendingpayment ;
	$discountamount = $resultarr[$p]['total_amt'] - $paid_pending ;
	$amtdate = date("Y-m-d", strtotime($resultarr[$p]['amt_date']));
	$bookqty = $resultarr[$p]['book_qty'];
	$totalamt = $resultarr[$p]['total_amt'];
	
	$str[$p]='<tr nobr="true">
		<td>'.$amtbookid.'</td>
		<td align="center">'.$fullname.'</td>
		<td align="center">'.$paidamount.'</td>
		<td align="center">'.$pendingpayment.'</td>
		<td align="center">'.$discountamount.'</td>
		<td align="center">'.$amtdate.'</td>
		<td align="right">'.$bookqty.'</td>
