...
class model_Post_default extends JDatabaseMySQL{  			
          var $qry = '';  
		  var $_result = '';             
		  var $searchCondition = ''; 
		   var $originalBanner = '';
          ...
          function __construct(  ){ 
			...
			$_REQUEST['creteria'] = substr($_REQUEST['creteria'],0,60); 
					...
					if(isset($_REQUEST['creteria']) && $_REQUEST['creteria']!='' && strtolower($_REQUEST['creteria'])!='search' ){
						$this->searchCondition .= ' and (post_title  like "%'.trim($_REQUEST['creteria']).'%"  )';
					}
				...
				$this->where .=' order by topic_id desc';
			   	$this->adjacents = '1';
				$this->page = 'index.php?mod=mod_Post&view=default&d=5&d=1&type=forum';
				$this->pageLimit = '10';
				...
				function deleteData($ids){
					$this->qry = "delete FROM posts WHERE post_id  in(".$ids.") ";
					$this->sql = $this->qry;
					if($this->query()){
						$this->qry = "delete FROM posts_image WHERE post_id  in(".$ids.") ";
						$this->sql = $this->qry;
						$this->query();
						return true;
					}else{
				...
				//echo $type; die;
					...
					$this->qry = "update posts set status='".$typeval."' WHERE post_id in(".$modelIds.") ";
					$this->sql = $this->qry;
		...
		function gettopic_image($topic_id){
			$this->sql = "SELECT * FROM posts_image where post_id='".$topic_id."'";	
			$this->query();
			return $this->loadAssoc();
		}
		
		function gettopic($topic_id){
			$this->sql = "SELECT * FROM topics where topic_id='".$topic_id."'";	
			$this->query();
			return $this->loadAssoc();
		}
