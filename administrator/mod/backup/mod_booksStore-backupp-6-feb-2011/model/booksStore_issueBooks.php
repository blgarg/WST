...
class model_booksStore_issueBooks extends JDatabaseMySQL{  			
          var $qry = '';  
		  var $_result = '';             
		  var $searchCondition = ''; 
		   var $originalBanner = '';
          ...
          function __construct(  ){ 
			...
			function initialize(){
				...
				if(isset($_REQUEST['issue_booksSearch']) && $_REQUEST['issue_booksSearch']=='issue_booksSearch'){
				...
				else if(isset($_REQUEST['issue_booksSearchBookWriters']) && $_REQUEST['issue_booksSearchBookWriters']=='issue_booksSearchBookWriters'){
					...
					if(isset($_REQUEST['businessid']) && isset($_REQUEST['partyid'])){
					if($_REQUEST['businessid']=='hga')
					{
					$this->searchCondition .= ' and Actions=1 and business_id=0';
					}
					else if(base64_decode($_REQUEST['partyid'])=='')
					{
					$this->searchCondition .= ' and Actions=1 and business_id='.base64_decode($_REQUEST['businessid']);
					
					}
					else
					{
					 $this->searchCondition .= ' and Actions=1 and business_id='.base64_decode($_REQUEST['businessid']).' and party_id='.base64_decode($_REQUEST['partyid']);
				...
				$this->adjacents = '1';
				if(isset($_REQUEST['issue_booksSearch']) && $_REQUEST['issue_booksSearch']=='issue_booksSearch'){
				$this->page = 'index.php?mod=mod_booksStore&view=issueBooks&m=4&creteria='.trim($_REQUEST['creteria']).'&issue_booksSearch='.$_REQUEST['issue_booksSearch']."&cmbStatus=".$_REQUEST['cmbStatus'];
				}else if(isset($_REQUEST['issue_booksSearchBookWriters']) && $_REQUEST['issue_booksSearchBookWriters']=='issue_booksSearchBookWriters'){
				$this->page = 'index.php?mod=mod_booksStore&view=issueBooks&m=4&creteria='.trim($_REQUEST['creteria']).'&issue_booksSearchBookWriters='.$_REQUEST['issue_booksSearchBookWriters']."&royalitywriter=".$_REQUEST['royalitywriter'];
				}
				else if(isset($_REQUEST['issue_booksSearchBookCategories']) && $_REQUEST['issue_booksSearchBookCategories']=='issue_booksSearchBookCategories'){
				$this->page = 'index.php?mod=mod_booksStore&view=issueBooks&m=4&creteria='.trim($_REQUEST['creteria']).'&issue_booksSearchBookCategories='.$_REQUEST['issue_booksSearchBookCategories']."&bookcategory=".$_REQUEST['bookcategory'];
				}
				else if(isset($_REQUEST['issue_booksSearchBusiness']) && $_REQUEST['issue_booksSearchBusiness']=='issue_booksSearchBusiness'){
				if(isset($_REQUEST['businessid']) && isset($_REQUEST['partyid'])){
					if(base64_decode($_REQUEST['partyid'])=='')
					{
					$this->page = 'index.php?mod=mod_booksStore&view=issueBooks&m=4&creteria='.trim($_REQUEST['creteria']).'&issue_booksSearchBusiness='.$_REQUEST['issue_booksSearchBusiness']."&businessid=".$_REQUEST['businessid']."&partyid=".$_REQUEST['partyid'];
					}
					else
					{
					$this->page = 'index.php?mod=mod_booksStore&view=issueBooks&m=4&creteria='.trim($_REQUEST['creteria']).'&issue_booksSearchBusiness='.$_REQUEST['issue_booksSearchBusiness']."&businessid=".$_REQUEST['businessid']."&partyid=".$_REQUEST['partyid'];
				...
				$this->page = 'index.php?mod=mod_booksStore&view=issueBooks&m=4';
				}
				$this->pageLimit = '10';
