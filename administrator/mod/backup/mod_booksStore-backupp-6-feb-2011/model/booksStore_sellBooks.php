...
class model_booksStore_sellBooks extends JDatabaseMySQL{  			
          var $qry = '';  
		  var $_result = '';             
		  var $searchCondition = ''; 
		   var $originalBanner = '';
		...
		function get_all_bookswriters()
		{
			...
			function initialize(){
				...
				if(isset($_REQUEST['businessid']) && isset($_REQUEST['partyid'])){
					...
					{
					$this->page = 'index.php?mod=mod_booksStore&view=sellBooks&m=2&creteria='.trim($_REQUEST['creteria']).'&issue_booksSearchBusiness='.$_REQUEST['issue_booksSearchBusiness']."&businessid=".$_REQUEST['businessid']."&partyid=".$_REQUEST['partyid'];
					}
					else
					{
					$this->page = 'index.php?mod=mod_booksStore&view=sellBooks&m=2&creteria='.trim($_REQUEST['creteria']).'&issue_booksSearchBusiness='.$_REQUEST['issue_booksSearchBusiness']."&businessid=".$_REQUEST['businessid']."&partyid=".$_REQUEST['partyid'];
