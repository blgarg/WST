...
//Check edit page
function checkEditTravel()
{
with (window.document.adddPageFrm) { 
		if (isEmpty(place_name, 'Please enter place name')) {
		
		return false;
		}
		else if(onlytext(place_name,'Please enter text in place name')){
		place_name.focus();
		return false;
		}else if(validateHtml(place_name,'Special characters are not allowed in place name')){
			return false;
		}
		else if(isEmpty(address,'Please enter the address'))
		{
			return false;
		}else if(validateHtml(address,'Special characters are not allowed in address')){
			return false;
		}
		else if(isEmpty(city,'Please enter the city'))
		{
			return false;
		}
		else if(onlytext(city,'Please enter text in city'))
		{
			city.focus();
			return false;
		}else if(validateHtml(city,'Special characters are not allowed in city')){
			return false;
		}
		else if(isEmptySelect(country_name, 'Please select country')) {
		return false;
		}
		else if(isEmptySelect(type, 'Please select type')) {
		return false;
		}else if(document.getElementById("editVideo").checked)
		{
			...
			else if(isValidImage(upload_image, 'Please select only jpg,gif,png and jpeg image.')) {
				return false;
...
function checkAddtravel()
{
	with (window.document.adddPageFrm) { 
		...
		return false;
		}else if(insertValidImage(image_load,'Please select valid image like jpg,jpeg,gif and png .')){
			return false;
		}else if(loadfilesize.value>1){
				document.getElementById("err_mess").innerHTML = "Image size should be less then 1 MB";
				document.getElementById("content-table-inner").style.display="block";
