...
class model_reports_giftbook extends JDatabaseMySQL{  			
          var $qry = '';  
		  var $_result = '';             
		  var $searchCondition = ''; 
		   var $originalBanner = '';
		...
		function get_all_bookswriters()
		{
			...
			function initialize(){
				...
				$this->tb_name='mgl_gift_books a,mgl_issue_book b,mgl_users as c';
				 $this->where = '  where b.issue_type=2 and a.gift_id=b.issue_foregin_id and c.user_id=a.issue_userid and '.$this->searchCondition.'  group by b.issue_foregin_id order by a.gift_id desc';
				...
				if(isset($_REQUEST['particularsearch']) && $_REQUEST['particularsearch']=='particularsearch'){
					...
					if(isset($_REQUEST['sdate']) && $_REQUEST['sdate']!=''){
						$this->page = 'index.php?mod=mod_reports&view=giftbook&m=6&filtertype='.$_REQUEST['filtertype'].'&report_datewise='.$_REQUEST['report_datewise'].'&sdate='.$_REQUEST['sdate'].'&edate='.$_REQUEST['edate']."&cmbStatus=".$_REQUEST['cmbStatus'];
					}
				}
				else if(isset($_REQUEST['report_yearwise']) && @$_REQUEST['report_yearwise']=='report_yearwise'){
				$year1=$_REQUEST['yearselection'];
				...
				$secondhalf_end=$year1."-12-31";
					if(isset($_REQUEST['typeselection']) && @$_REQUEST['typeselection']==3){
					$this->page = 'index.php?mod=mod_reports&view=giftbook&m=6&yearselection='.$_REQUEST['yearselection'].'&report_yearwise='.$_REQUEST['report_yearwise'].'&typeselection='.$_REQUEST['typeselection'];
					}
					else if(isset($_REQUEST['typeselection']) && @$_REQUEST['typeselection']==1){
								if(isset($_REQUEST['quarter']) && @$_REQUEST['quarter']==1){
								$this->page = 'index.php?mod=mod_reports&view=giftbook&m=6&yearselection='.$_REQUEST['yearselection'].'&report_yearwise='.$_REQUEST['report_yearwise'].'&quarter='.$_REQUEST['quarter'].'&typeselection='.$_REQUEST['typeselection'];
						 }
						 else if(isset($_REQUEST['quarter']) && @$_REQUEST['quarter']==2){
								$this->page = 'index.php?mod=mod_reports&view=giftbook&m=6&yearselection='.$_REQUEST['yearselection'].'&report_yearwise='.$_REQUEST['report_yearwise'].'&quarter='.$_REQUEST['quarter'].'&typeselection='.$_REQUEST['typeselection'];
						 }
						...
						if(isset($_REQUEST['selecthalf']) && @$_REQUEST['selecthalf']==1){
								$this->page = 'index.php?mod=mod_reports&view=giftbook&m=6&yearselection='.$_REQUEST['yearselection'].'&report_yearwise='.$_REQUEST['report_yearwise'].'&selecthalf='.$_REQUEST['selecthalf'].'&typeselection='.$_REQUEST['typeselection'];
						 }
						 else if(isset($_REQUEST['selecthalf']) && @$_REQUEST['selecthalf']==2){
								$this->page = 'index.php?mod=mod_reports&view=giftbook&m=6&yearselection='.$_REQUEST['yearselection'].'&report_yearwise='.$_REQUEST['report_yearwise'].'&selecthalf='.$_REQUEST['selecthalf'].'&typeselection='.$_REQUEST['typeselection'];
						 }
				...
				$this->page = 'index.php?mod=mod_reports&view=giftbook&m=6';
				}
				$this->pageLimit = '10';
		...
		{
			$this->sql = "SELECT sum(book_qty) as totalbooks FROM mgl_issue_book where issue_type=2 and issue_foregin_id='$id'";	
			$this->query();
			return $this->loadAssoc();
		}
