...
$string='<table border="1" cellpadding="2" cellspacing="2" >
 <tr nobr="true" style="background-color:#E8E8E4;" align="center">
  <th width="200px;"><b>('.typetransection.')</b></th>
  <th width="100px;">'.EMPNAME.'</th>
...
for($p=0;$p<count($resultarr);$p++){
	...
	if(!isset($resultarr[$p]['receivepmt']) && $resultarr[$p]['receivepmt'] ==''){
		$typeoftransection = "Sell of Books";
		$amtdate = $resultarr[$p]['amt_date'];
		$qty = $resultarr[$p]['book_qty'];
		$totalamt = $resultarr[$p]['total_amt_withanothercost'];
	} else{
		$typeoftransection = "Payment received";
		$amtdate = date("Y-m-d", strtotime($resultarr[$p]['createDate']));;
		$qty = "-";
		$totalamt = $resultarr[$p]['receivepmt'];
	}
	
	$str[$p]='<tr nobr="true">
		<td>'.$typeoftransection.'</td>
