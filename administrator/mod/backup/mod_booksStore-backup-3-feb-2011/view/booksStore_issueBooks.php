	  ...
	  if(obj.readyState==4)
        ...
        <div id="content-table-inner">
			...
			<form name="searchForm" id="searchForm" method="get" >
                    ...
                    <option value="1" <?php if(isset($_REQUEST['businessid']) && $_REQUEST['businessid']!='') echo 'selected'; else ''; ?> >Business</option>
                   ...
                   <input type="hidden" name="issue_booksSearchBusiness" value="issue_booksSearchBusiness" />
                    ...
                    <input type="hidden" name="view" value="issueBooks"  />
                    <input type="hidden" name="m" value="4"  />
                    <tr>
                    <td valign="top" style="padding-right:10px;"><select  class="selectbox" name="businessid" id="businessid" style="width:100%;padding:3px 0 0 6px;" onchange="get_parties(this.value)"  >
                    <option value="<?php base64_encode(0);?>" <?php if(isset($_REQUEST['cmbStatus']) && $_REQUEST['cmbStatus']=='') echo 'selected'; else ''; ?> >Select Business type</option>
                   <?php
				   $all_busin_cate=new model_booksStore_issueBooks();
				   $all_business=$all_busin_cate->get_all_businesscategories();
				   for($i=0;$i<count($all_business);$i++)
				   {
				   $business_id=base64_decode(@$_REQUEST['businessid']);
				   ?>
                    <option value="<?php echo base64_encode($all_business[$i]['buiness_cat_id']);?>" <?php if(isset($_REQUEST['businessid']) && $_REQUEST['businessid']!='' && $business_id==$all_business[$i]['buiness_cat_id']) echo 'selected'; ?> ><?php echo $all_business[$i]['business_cat_title']?></option>
                   <?php
				   }
				   ?>
                    </select> </td>
                    <td valign="top"><select  class="selectbox" name="partyid" id="partyid" style="width:100%;padding:3px 0 0 6px;"  ><option value="<?php base64_encode(0);?>">Select Party</option>
                    <option value="<?php base64_encode(0);?>">No Record Found</option>
                    </select> </td>
                    <td valign="top">
                    <input type="submit"  class="button_r"   value="Filter" onclick="this.form.submit();" style="margin-left:10px;" />
                    </td>
                    </tr></form>
                    </table>
                   ...
                   <input type="hidden" name="issue_booksSearchBookCategories" value="issue_booksSearchBookCategories" />
                    ...
                    <input type="hidden" name="m" value="4"  />
                    <tr>
                    <td valign="top"><select  class="selectbox" name="bookcategory" style="width:100%;padding:3px 0 0 6px;"  >
                    <option value="0" <?php if(isset($_REQUEST['cmbStatus']) && $_REQUEST['cmbStatus']=='') echo 'selected'; else ''; ?> >Select Book Categories</option>
                   <?php
				   $all_book_cate=new model_booksStore_issueBooks();
                   ...
                   <input type="hidden" name="issue_booksSearchBookWriters" value="issue_booksSearchBookWriters" />
                    <input type="hidden" name="mod" value="mod_booksStore"  />
                    <input type="hidden" name="view" value="issueBooks"  />
                    <input type="hidden" name="m" value="4"  />
                    <tr>
                    <td valign="top"><select  class="selectbox" name="royalitywriter" style="width:100%;padding:3px 0 0 6px;"  >
                    <option value="0" <?php if(isset($_REQUEST['cmbStatus']) && $_REQUEST['cmbStatus']=='') echo 'selected'; else ''; ?> >Select Writers</option>
                   <?php
				   $all_book_writer=new model_booksStore_issueBooks();
