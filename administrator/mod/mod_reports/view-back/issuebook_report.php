...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        // Title
		$title=HARYANAGRANTHACADEMY."(".ISSUEBOOKREPORT.")";
...
$string='<table border="1" cellpadding="2" cellspacing="2" >
 <tr nobr="true" style="background-color:#E8E8E4;" align="center">
  <th width="150px;">'.EMPLOYEENAME.'</th>
  <th width="60px;">Issue Order Number</th>
  <th width="40px;">Total Books</th>
  <th width="50px;">Issued Date</th>
  <th  width="50px;">Issued Year</th>
  <th  width="140px;">Enter Employee</th>
...
$totalbooks=get_totalbooks($row['amt_id']);
$get_all_name=get_all_name($row['issue_user_id']);
$get_all_emp=get_all_name($row['emp_issue_id']);
//$partytitle=get_all_party($row['party_id']);
...
$pdf->Output('issuebookreport.pdf', 'I');
...
return $row['user_display_name'];
 	...
 	function get_totalbooks($id)
		{
			$str= "SELECT sum(book_qty) as totalbooks FROM mgl_issue_book where issue_type=1 and issue_foregin_id='$id'";	
			$rs=mysql_query($str);
			$row=mysql_fetch_array($rs);
...
$Session = new session();
$str=$_REQUEST['str'];
$contents="<span style='font-weight:bold'>Issue Employee name</span>,<span style='font-weight:bold'>Issue Order number</span>,<span style='font-weight:bold'>Total Books</span>,<span style='font-weight:bold'>Issue Date</span>,<span style='font-weight:bold'>Issue Year</span>,<span style='font-weight:bold'>Enter Employee</span>\n\n";
$user_query = mysql_query("$str");
while($row = mysql_fetch_array($user_query))
{
$contents.=$row['issue_user_id'].",";
$contents.=$row['amt_id'].",";
$contents.=$row['amt_id'].",";
$contents.=$row['amt_date'].",";
$contents.=$row['amt_year'].",";
$contents.=$row['emp_issue_id']."\n";;
}
