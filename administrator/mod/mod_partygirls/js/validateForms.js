...
//Check edit page
function checkEditgallery()
{
with (window.document.adddPageFrm) {
		if (isEmpty(gallery_title, 'Please enter gallery title')) {
			return false;
		}else if(onlytext(gallery_title, 'Please enter text only.')) {
			return false;
		}
		else if(validateHtml(gallery_title,'Special characters are not allowed in gallery title')){
			return false;
		}
		...
		else if(document.getElementById("editVideo").checked)
		{
			...
			else if(isValidImage(upload_video, 'Please select only jpg,gif,png and jpeg image.')) {
				return false;
			}else if(loadfilesize.value>5){
				document.getElementById("err_mess").innerHTML = "Image size should be less then 5 MB";
				document.getElementById("content-table-inner").style.display="block";
				return false;	
		}else {
			submit();
...
function checkAddgallery()
{
	with (window.document.adddPageFrm) { 
		...
		else if (isEmptyImage(image_load, 'Kindly select the image.')) {
		return false;
		}
		else if (isValidImage(image_load, 'Invalid image.')) {
		return false;
		}else if(loadfilesize.value>5){
				document.getElementById("err_mess").innerHTML = "Image size should be less then 5 MB";
				document.getElementById("content-table-inner").style.display="block";
