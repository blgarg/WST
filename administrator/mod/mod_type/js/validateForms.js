...
with (window.document.adddPageFrm) { 
		...
		else if(onlytext(gallery_title,'Please enter the text only')){
		gallery_title.focus();
		return false;
		}
		...
		else if(document.getElementById("editVideo").checked)
		{
			...
			else if(isValidImage(upload_video, 'Invalid image.')) {
				return false;
			}else {
			submit();
		}	
...
{
	with (window.document.adddPageFrm) {  
	    if (isEmpty(type_title, 'Please enter title ')) {
		
		return false;
		}
		else if(onlytext(type_title,'Please enter only text in title'))
		{
			return false; 
		}else if(validateHtml(type_title,'Special characters are not allowed in title')){
			return false;
		}
		else {
			submit();
