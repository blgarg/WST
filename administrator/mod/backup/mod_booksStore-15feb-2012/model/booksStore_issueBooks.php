...
class model_booksStore_issueBooks extends JDatabaseMySQL{  			
          ...
          function __construct(  ){ 
			...
			function initialize(){
				if(isset($_REQUEST['particularsearch']) && $_REQUEST['particularsearch']=='particularsearch' && $_REQUEST['creteria']!='' && !(isset($_REQUEST['businessid']))){
					if(isset($_REQUEST['cmbStatus']) && $_REQUEST['cmbStatus']!=''){
						$this->searchCondition .= ' and Actions=1';
					}
					if(isset($_REQUEST['creteria']) && $_REQUEST['creteria']!=''){
				...
				if(isset($_REQUEST['businessid'])){
					$this->page = 'index.php?mod=mod_booksStore&view=issueBooks&m=4&creteria='.trim($_REQUEST['creteria']).'&issue_booksSearchBusiness='.$_REQUEST['issue_booksSearchBusiness']."&partyid=".$_REQUEST['businessid'];		}			
				}
				else{
