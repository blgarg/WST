...
$CatDb_OB2 = new JDatabaseMySQL();
$CatDb_OB2->sql = "SELECT * FROM topics where topic_id='".$_REQUEST['topic_id']."'";	
$CatDb_OB2->query();
$CatDb_OB2->getNumRows();
$forum_data = $CatDb_OB2->loadAssoc();

$CatDb_OB3 = new JDatabaseMySQL();
$CatDb_OB3->sql = "SELECT * FROM topic_image where topic_id='".$_REQUEST['topic_id']."'";	
$CatDb_OB3->query();
$CatDb_OB3->getNumRows();
$image_data = $CatDb_OB3->loadAssoc();
?>
<div id="content-table-inner" class="Add_New_Web_Page">
	 <form action="./index.php?mod=mod_forum&view=editForum&d=1&type=forum" onsubmit=" return checkAddCategory();" method="post" id="adddPageFrm" name="adddPageFrm" enctype="multipart/form-data"  onSubmit="return checkAddmodel();" >
   ...
   <td style="width:73%">
    <table border="0" cellpadding="0" cellspacing="0"  id="id-form">
            <tr>
              ...
              <td width="558"><input type="text"  name="title" value="<?php echo $forum_data[0]['topic_title']; ?>"  id="category_title" maxlength="50" class="textbox"/></td>
            </tr>
			 <tr>
			  ...
			  <td>
                    	<select name="cat" style="width:90%;border: 1px solid #D7D7D7;font-size: 14px;height: 25px;line-height: 24px;" id="cat" onchange="return subcatfun(this.value);">
							...
							<option value="<?php echo $cat_data['cat_id']; ?>" <?php if(@$cat_data['cat_id']==$forum_data[0]['topic_cat']){?>selected="selected" <?php }?>><?php echo $cat_data['cat_name']; ?></option>
							<?php }?>
                        </select>
                    </td>
            </tr>
		 ...
		 <tr>
			  ...
			  <td id="subcat">
                    	<select name="sub_cat" style="width:90%;border: 1px solid #D7D7D7;font-size: 14px;height: 25px;line-height: 24px;" id="sub_cat">
							...
							<option value="<?php echo $sub_cat_data['sub_cat_id']; ?>" <?php if(@$sub_cat_data['sub_cat_id']==$forum_data[0]['topic_subcat']){?>selected="selected" <?php }?>><?php echo $sub_cat_data['sub_cat_name']; ?></option>
							<?php }?>
                        </select>
                    </td>
            </tr>
	  ...
	  $obj->Height=400;
	  $obj->Width=800;
	  $obj->Value= @$forum_data[0]['topic_desc'];
	  $obj->Create();
	  ?><?php //echo $code = $CKEditor->editor("categoryText", @$page_content,$config);?></td>
            </tr> 
			<tr><th width="153" valign="top">Image :</th>
				<td><?php foreach($image_data as $image){?><img src="<?php  echo $config_var->UPLOAD_URL."/forum/thumbnail/".$image['image_name'];?>" alt="Smiley face" height="175" width="175"><?php }?></td>
				</tr>
			<tr>
            ...
            <th>&nbsp;</th>
            <td valign="top">
                <input type="hidden" name="controller" value="editForum"  />
