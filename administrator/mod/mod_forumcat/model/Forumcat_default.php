<?php
class model_Forumcat_default extends JDatabaseMySQL{  			
          var $qry = '';  
		  var $_result = '';             
		  var $searchCondition = ''; 
		   var $originalBanner = '';
          ...
          function __construct(  ){ 
			...
			$_REQUEST['creteria'] = substr($_REQUEST['creteria'],0,60); 
					...
					if(isset($_REQUEST['creteria']) && $_REQUEST['creteria']!='' && strtolower($_REQUEST['creteria'])!='search' ){
						$this->searchCondition .= ' and (cat_name  like "%'.trim($_REQUEST['creteria']).'%"  )';
					}
				...
				$this->where .=' order by cat_id desc';
			   	$this->adjacents = '1';
				...
				function deleteData($ids){
					$this->qry = "delete FROM forum_categories WHERE cat_id  in(".$ids.") ";
					$this->sql = $this->qry;
				...
				//echo $type; die;
					...
					$this->qry = "update forum_categories set status='".$typeval."' WHERE cat_id in(".$modelIds.") ";
					$this->sql = $this->qry;
