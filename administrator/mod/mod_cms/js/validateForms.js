...
function pageAction(controller)
{
	...
	var chkboxlen = document.getElementsByName('model_id[]').length;
	
	for(var i=0;i<chkboxlen;i++){
		if (document.getElementsByName('model_id[]')[i].checked){
			checkFound = true;
		}
	}
...
function checkAddPage()
{
	with (window.document.adddPageFrm) { 
		if(isEmpty(english_title,'Please enter the page title')){
			return false;
		}else if(onlytext(english_title,'Please enter text only for page title.')){
			return false;
		}else if(validateHtml(english_title,'Special characters are not allowed in page title')){
			return false;
		}else if(isEmpty(page_meta_tags,'Please enter the meta title.')){
			return false;
		}else if(onlytext(page_meta_tags,'Please enter text only for meta title.')){
			return false;
		}else if(validateHtml(page_meta_tags,'Special characters are not allowed in meta title')){
			return false;
		}else if(isEmpty(meta_desc,'Please enter the meta description.')){
			return false;
		}else if(validateHtml(meta_desc,'Special characters are not allowed in meta description')){
			return false;
		}else{
			submit();
		}
