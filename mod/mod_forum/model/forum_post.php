...
class model_forum_post extends JDatabaseMySQL{  			
          var $qry = '';  
		  var $_result = '';                     
          function __construct( $options='' ){ }   			   		   
		  function initialize(){
				$this->tb_name = 'topics';
				$this->where = ' where 1 and status=1 and topic_by!="0" and topic_subcat="'.$_REQUEST['sub_cat'].'" ';
					...
					$fileinfo = pathinfo($file['name'][$a]);
						...
						if(($fileinfo['extension']=="PNG" || $fileinfo['extension']=="png"|| $fileinfo['extension']=="JPG"|| $fileinfo['extension']=="jpg"|| $fileinfo['extension']=="gif"|| $fileinfo['extension']=="GIF"|| $fileinfo['extension']=="jpeg"|| $fileinfo['extension']=="JPEG") && $file['size'][$a]/(1024*1024) < 5)
						{
							...
							if(move_uploaded_file($file['tmp_name'][$a],$config_var->UPLOAD_ROOT."forum/".$filename))
							{
								...
								$thumb = $model_Forum_post->createthumbnail($url,$thumbnail_path,$filename,'175',$file['type'][$a]);
								$data['error'][$a] = "";
			...
			function inserData($data)
			{
			  ...
			  $this->subcategory = @mysql_real_escape_string(stripslashes($data->subcategory));
			   $this->topic_id = @mysql_real_escape_string(stripslashes($data->topic_id));
			  $this->createdDate = date('Y-m-d g:i:s');
			  
			  	
			
			$this->qry = "INSERT INTO posts(
					  post_title,
					  post_content,
					  cat_id,
					  sub_cat_id,
				...
				$this->sql = $this->qry;
				if($this->query()){ 
				$post_id= $this->insertid();
				foreach($data->filename as $filename)
				{
					$this->qry1 = "INSERT INTO posts_image(
					  post_id,
					  topic_id,
					  image_name,
					  status) VALUES
