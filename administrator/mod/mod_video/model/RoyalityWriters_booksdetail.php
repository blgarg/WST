...
class model_RoyalityWriters_booksdetail extends JDatabaseMySQL{  			
          var $categoryTitle;
		  var $categoryText;
          ...
          function initialize(){ return ; }	
		   function get_order_detail($id)
			...
			return($row);
				...
				{
				$this->sql = "SELECT * FROM mgl_sell_books WHERE sell_amt_id='".base64_decode($id['transaction_id'])."'";
				$this->query();
				$this->_result = $this->loadAssoc();   				
				return $this->_result;
				}
				function get_all_taxes_detail($id)									
				{
				$this->sql = "SELECT * FROM mgl_amt_sell_books WHERE amt_book_id='".base64_decode($id['transaction_id'])."'";
				$this->query();
				$this->_result = $this->loadAssoc();   				
				return $this->_result;
				}
