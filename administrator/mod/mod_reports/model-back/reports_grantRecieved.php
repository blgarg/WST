...
class model_reports_grantRecieved extends JDatabaseMySQL{  			
          var $qry = '';  
		  var $_result = '';             
		  var $searchCondition = ''; 
		   var $originalBanner = '';
			...
			function initialize(){
				...
				if(isset($_REQUEST['particularsearch']) && $_REQUEST['particularsearch']=='particularsearch'){
					...
					if(isset($_REQUEST['sdate']) && $_REQUEST['sdate']!=''){
						...
						$this->searchCondition .= " a.created_date>='".$_REQUEST['sdate']."' and a.created_date<='".$_REQUEST['edate']."' ";
					}
				}
				...
				$secondhalf_end=$year1."-12-31";
				 ...
				 $this->where = ' mgl_grant a,mgl_users b where a.enteruser=b.user_id and '.$this->searchCondition.'  order by a.grand_id desc';
				...
				if(isset($_REQUEST['report_datewise']) && $_REQUEST['report_datewise']=='report_datewise'){
					...
					if(isset($_REQUEST['sdate']) && $_REQUEST['sdate']!=''){
										$this->page = 'index.php?mod=mod_reports&view=grantRecieved&m=6&d=13&filtertype='.$_REQUEST['filtertype'].'&report_datewise='.$_REQUEST['report_datewise']."&sdate=".$_REQUEST['sdate']."&edate=".$_REQUEST['edate'];
					}
				}
				else if(isset($_REQUEST['report_yearwise']) && @$_REQUEST['report_yearwise']=='report_yearwise'){
				$year1=$_REQUEST['yearselection'];
				...
				$secondhalf_end=$year1."-12-31";
					if(isset($_REQUEST['typeselection']) && @$_REQUEST['typeselection']==3){
						$this->page = 'index.php?mod=mod_reports&report_yearwise='.$_REQUEST['report_yearwise'].'&view=grantRecieved&m=6&d=13&filtertype='.$_REQUEST['filtertype'].'&yearselection='.$_REQUEST['yearselection'].'&typeselection='.$_REQUEST['typeselection'];
					}
					else if(isset($_REQUEST['typeselection']) && @$_REQUEST['typeselection']==1){
								if(isset($_REQUEST['quarter']) && @$_REQUEST['quarter']==1){
								$this->page = 'index.php?mod=mod_reports&view=grantRecieved&m=6&d=13&yearselection='.$_REQUEST['yearselection'].'&report_yearwise='.$_REQUEST['report_yearwise']."&typeselection=".$_REQUEST['typeselection']."&quarter=".$_REQUEST['quarter'];
						 }
						 else if(isset($_REQUEST['quarter']) && @$_REQUEST['quarter']==2){
									$this->page = 'index.php?mod=mod_reports&view=grantRecieved&m=6&d=13&yearselection='.$_REQUEST['yearselection'].'&report_yearwise='.$_REQUEST['report_yearwise']."&typeselection=".$_REQUEST['typeselection']."&quarter=".$_REQUEST['quarter'];
						 }
						...
						if(isset($_REQUEST['selecthalf']) && @$_REQUEST['selecthalf']==1){
								...
								$this->page = 'index.php?mod=mod_reports&view=grantRecieved&m=6&d=13&yearselection='.$_REQUEST['yearselection'].'&report_yearwise='.$_REQUEST['report_yearwise']."&typeselection=".$_REQUEST['typeselection']."&selecthalf=".$_REQUEST['selecthalf'];
						 }
						 else if(isset($_REQUEST['selecthalf']) && @$_REQUEST['selecthalf']==2){
								$this->page = 'index.php?mod=mod_reports&view=grantRecieved&m=6&d=13&yearselection='.$_REQUEST['yearselection'].'&report_yearwise='.$_REQUEST['report_yearwise']."&typeselection=".$_REQUEST['typeselection']."&selecthalf=".$_REQUEST['selecthalf'];
						 }
				...
				$this->page = 'index.php?mod=mod_reports&view=grantRecieved&m=6&d=13';
				}
				$this->pageLimit = '10';
				//chcecked
